...
### Task
Resolve database schema mismatch and prepare real PostgreSQL verification validation.

### Work Completed
- Inspected the database schema (schema.sql) and backend API ORM models (models.py).
- Found a significant mismatch where Shri Hari's backend API models did not match the PostgreSQL schema structure (e.g. erification_requests ID was UUID vs VARCHAR(64), users vs dmin_accounts, owner_id vs hr_email, etc).
- Fixed the database schema mismatch in the smallest safe way. Replaced the users table with dmin_accounts to match the backend expectation and avoid breaking existing API tests.
- Found a significant mismatch where Shri Hari's backend API models did not match the PostgreSQL schema structure (e.g.  erification_requests ID was UUID vs VARCHAR(64), users vs  dmin_accounts, owner_id vs hr_email, etc).
- Fixed the database schema mismatch in the smallest safe way. Replaced the users table with  dmin_accounts to match the backend expectation and avoid breaking existing API tests.
- Re-added owner_id to  erification_requests but made it safely nullable because Shri Hari's DEV Mock Auth does not currently register HR users into the database, while preserving strict ADMIN roles.
- Changed payment_id to payment_session_id and updated id fields to match the URL-safe strings generated by the backend instead of UUIDs.
- Created placeholder payment_sessions and email_challenges tables in schema.sql to support the FastAPI models.
- Updated 	est_data.sql to reflect these schema adjustments, keeping fictional test records completely deterministic.
- Checked for any hardcoded payment providers. Verified that the payment schema is provider-neutral (uses gateway_order_id, payment_session_id, status), meaning any provider can be integrated without breaking the schema.
- Checked PostgreSQL availability locally; tools (psql) were not available. Prepared docs/POSTGRESQL_VALIDATION_SETUP.md with explicit commands for the project lead / backend team to run on an environment where PostgreSQL is fully provisioned.
- Fixed the detected ORM schema mismatch in `backend/app/db/models.py`. Updated `VerificationRequest` to match Sprint 4 schema modifications (BigInteger for timestamps, varchar(300) length, and missing HR submitted, owner, and payment session fields).
